...
from sklearn.linear_model import LinearRegression #linear regression for coefficient strength
import tensorflow as tf #machine learning libraries for model creation
...
from sklearn.model_selection import train_test_split #split data into training, testing, validation
...
from keras import optimizers #optimizer for deep learning model (most likely Adam)

def plot():
    indices = ['QQQ','SPY','SPXL','DIA','VTWO'] #etfs that track NASDAQ 100, S&P 500, 3x Daily S&P 500, Dow Jones, Russell 2000
    for index in indices: #loop through ETFs
        df = web.DataReader(index,'yahoo','2010-01-01',dt.datetime.now())[['Adj Close']] #adjusted close from 2010 
        df.plot() 
        plt.title(f'{index} Returns') #title is ETF ticker
        plt.ylabel('ETF Price') #y axis title
        plt.show() #show plot
        
def significant():
    df = web.DataReader('^GSPC','yahoo','2010-01-01',dt.datetime.now())[['Adj Close']] #adjusted close of S&P 500 index
    symbols_list = 'T10YIE','T5YIFR','DBAA','TEDRATE','MORTGAGE15US','OBMMIJUMBO30YF' #symbols used for macroeconomic daily updated indicators 
    symbols = [] #empty list
    for ticker in symbols_list: #looping through these economic indicators
        try:#try clause
            r = web.DataReader(ticker,'fred','2010-01-01',dt.datetime.now())  #extracting data from FRED API
            r[f'{ticker}'] = r[f'{ticker}']  #making a column for each indicator
            r1 = r[[f'{ticker}']].rolling(window=3).mean() #3 day simple moving average
            symbols.append(r1) #adding all indicators to one list
        except: #just in case the API doesn't have that tickers
            msg = 'Failed to read symbol: {0!r}, replacing with NaN.' #error message
    data = pd.concat(symbols, sort=False, axis=1)  #combining all the indicators to one dataframe
    data = data.fillna(method='ffill') #forward filling if any null values are left out
    data = data.dropna() #dropping any null values behind 
    df = pd.concat([data,df],axis=1).dropna() #combining ticker price data with macroeconomic stats data
    ...
    return df.corr(), sig #return correlation matrix and significant feature coefficients from linear regression to see strength

def data(ticker,start,end):
    df = web.DataReader(ticker.upper(),'yahoo',start,end)[['Adj Close']] #adjusted close of all the indices
    ...
    return df.dropna() #dataframe with no null values
...
def predict(df,pred_df):
    ...
    model.add(Dense(50, input_dim=x_train.shape[1], #need the input shape of the data in tensorflow 2
    ...
    model.add(Dense(1, activation=tf.nn.leaky_relu, #only one answer so you need one unit
                        kernel_initializer='he_normal'))
    ...
    history = model.fit(x_train, y_train,  #fit the training data to the model
                        ...
                        batch_size=32, #batch size of 32 - pretty standard
    ...
    if float(prediction[0])>float(df['Target'].iloc[-1:].values): #buy or sell statement based on model prediction and real time tick data
        ...
        print('Sell: ', round(float(prediction[0]),2), '<',round(float(df['Target'].iloc[-1:].values),2),
              '\nPCT DIFF: ',round((float(prediction[0]) - float(df['Target'].iloc[-1:].values))/float(df['Target'].iloc[-1:].values)*100,2),'%')
    return mape #return mape for checking purpose
...
def bin_data(ticker,start,end):
    ...
    df['sm3'] = df['Adj Close'].rolling(window=3).mean() #3 day SMA of index
    df['sm9'] = df['Adj Close'].rolling(window=9).mean() #9 day SMA of index
    symbols_list = ['T10YIE','T5YIFR','DBAA'] #10yr BE IR / 5yr, 5yr forward inflation exp. rate/5-year Baa Corporate Bond Yield
    symbols = [] 
    for ticker in symbols_list: 
        try: 
            ...
            r1 = r[[f'{ticker}']].rolling(window=3).mean() #3 day sma feature
            symbols.append(r1) 
        except:
            msg = 'Failed to read symbol: {0!r}, replacing with NaN.'
    data = pd.concat(symbols, sort=False, axis=1) 
...
def bin_predict(df):
    ...
    acc = ["%.2f" % i for i in acc] #rounding it to 2 decimals
    print(f"Validation Accuracy {acc}") #print the accuracy list first epoch first

        
if __name__ == '__main__':
    plot() #plotting indices' prices from 2010/01/01
    ...
    for ticker in tickers: #looping through indices / etfs
        ...
        try: #try clause just in case API doesn't have that ticker
            ...
            bin_predict(bin_data(ticker,start,end)) #we want to predict the ind. indices binary returns
